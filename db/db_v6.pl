    ...
    my $mapping = '
{
    ...
    esPut("/${PREFIX}sequence_v30/_mapping/_doc?master_timeout=${ESTIMEOUT}s", $mapping);
}
################################################################################
sub sequenceUpgrade
{
    ...
    my $mapping = '
{
    ...
    esPut("/${PREFIX}files_v30/_mapping/_doc?master_timeout=${ESTIMEOUT}s", $mapping);
}
################################################################################
sub statsCreate
{
...
my $mapping = '
{
    ...
    esPut("/${PREFIX}stats_v30/_mapping/_doc?master_timeout=${ESTIMEOUT}s&pretty", $mapping, 1);
}
################################################################################
sub dstatsCreate
{
...
my $mapping = '
{
    ...
    esPut("/${PREFIX}dstats_v30/_mapping/_doc?master_timeout=${ESTIMEOUT}s&pretty", $mapping, 1);
}
################################################################################
sub fieldsCreate
{
    ...
    my $mapping = '
{
    ...
    esPut("/${PREFIX}fields_v30/_mapping/_doc?master_timeout=${ESTIMEOUT}s", $mapping);
    ...
    my $mapping = '
{
    ...
    esPut("/${PREFIX}queries_v30/_mapping/_doc?master_timeout=${ESTIMEOUT}s&pretty", $mapping);
  ...
  "order": 99
    ...
    if ($UPGRADEALLSESSIONS) {
        ...
        foreach my $i (keys %{$indices}) {
            progress("$i ");
            esPut("/$i/_mapping/_doc?master_timeout=${ESTIMEOUT}s", $mapping, 1);
        }
        logmsg "\n" if (scalar(keys %{$indices}) != 0);
    }
...
if ($UPGRADEALLSESSIONS) {
    ...
    foreach my $i (keys %{$indices}) {
        progress("$i ");
        esPut("/$i/history/_mapping/_doc?master_timeout=${ESTIMEOUT}s&include_type_name=true", $mapping, 1);
    }
    logmsg "\n" if (scalar(keys %{$indices}) != 0);
}
...
esPut("/${PREFIX}hunts_v30/_mapping/_doc?master_timeout=${ESTIMEOUT}s&pretty", $mapping);
}
################################################################################
...
esPut("/${PREFIX}lookups_v30/_mapping/_doc?master_timeout=${ESTIMEOUT}s&pretty", $mapping);
}
################################################################################
    ...
    my $mapping = '
{
    ...
    esPut("/${PREFIX}users_v30/_mapping/_doc?master_timeout=${ESTIMEOUT}s&pretty", $mapping);
}
################################################################################
sub setPriority
{
...
if ($ARGV[1] =~ /^(init|wipe|clean)/) {
    ...
    foreach my $index (@indexes) { # import mappings
        if (-e "$ARGV[2].${PREFIX}${index}.mappings.json") {
            ...
            esPut("/$index[0]/$type[0]/_mapping/_doc?master_timeout=${ESTIMEOUT}s&pretty", to_json($mappings));
            close($fh);
        }
    }
    ...
    foreach my $template (@templates) { # update mappings
        if (-e "$ARGV[2].${PREFIX}${template}.template.json") {
            ...
            if (($template cmp "sessions2") == 0 && $UPGRADEALLSESSIONS) {
                ...
                foreach my $i (keys %{$indices}) {
                    progress("$i ");
                    esPut("/$i/session/_mapping/_doc?master_timeout=${ESTIMEOUT}s&include_type_name=true", to_json($mapping), 1);
                }
                logmsg "\n";
                ...
                foreach my $i (keys %{$indices}) {
                    progress("$i ");
                    esPut("/$i/_mapping/_doc?master_timeout=${ESTIMEOUT}s", to_json($mapping), 1);
                }
                logmsg "\n";
                ...
                foreach my $i (keys %{$indices}) {
                    progress("$i ");
                    esPut("/$i/history/_mapping/_doc?master_timeout=${ESTIMEOUT}s&include_type_name=true", to_json($mapping), 1);
                }
                logmsg "\n";
            }
            close($fh);
